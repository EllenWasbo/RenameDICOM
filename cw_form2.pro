...
;
; PURPOSE:
; cw_form_2 equals cw_form, but position for the form is screen center
;	cw_form_2 is a compound widget that simplifies creating
...
;		and its value returned as the result of cw_form_2.  For non-
...
;  *****************
;
;    To use cw_form_2 inside another widget:
;	    a = widget_base(title='Testing')
;	    b = cw_form_2(a, desc, /COLUMN)
...
;	   ID              LONG                <id of cw_form_2 widget>
...
function cw_form_2_PARSE, Extra, Name, Value, Index=Index
...
pro cw_form_2_APPEND, extra, e, keyword, USE_VALUE=use_value, ACTUAL_KEYWORD=akw
  COMPILE_OPT hidden

if cw_form_2_PARSE(e, keyword, value) then begin
...
pro cw_form_2_LABEL, parent, nparent, e, frame
;Put LABEL_LEFT and/or LABEL_RIGHT on a base.

  COMPILE_OPT hidden

nparent = parent
if cw_form_2_PARSE(e, 'LABEL_LEFT', value) then begin
	nparent = WIDGET_BASE(nparent, /ROW, FRAME=frame)
	frame = 0
	junk1 = WIDGET_LABEL(nparent, VALUE=value)
	endif
if cw_form_2_PARSE(e, 'LABEL_TOP', value) then begin
...
pro cw_form_2_BUILD, parent, desc, cur, ids, lasttag
...
;
  ...
  while cur lt n do begin
	...
	quit = cw_form_2_PARSE(e, 'QUIT')
	frame = cw_form_2_PARSE(e, 'FRAME')
	efn = cw_form_2_PARSE(e, 'EVENT', event_fun)
	cw_form_2_APPEND, extra, e, 'FONT', /USE_VALUE
	cw_form_2_APPEND, extra, e, 'COLUMN'
	cw_form_2_APPEND, extra, e, 'ROW'
	cw_form_2_APPEND, extra, e, 'LEFT', ACTUAL='ALIGN_LEFT'
	cw_form_2_APPEND, extra, e, 'CENTER', ACTUAL='ALIGN_CENTER'
	cw_form_2_APPEND, extra, e, 'RIGHT', ACTUAL='ALIGN_RIGHT'
    endif else e = ''
...
'BASE': BEGIN
    ...
    ENDCASE
'BUTTON': BEGIN
    cw_form_2_APPEND, extra, e, 'LABEL_LEFT', /USE_VALUE
    cw_form_2_APPEND, extra, e, 'LABEL_TOP', /USE_VALUE
    exclusive = cw_form_2_PARSE(e,'EXCLUSIVE')
    no_release = cw_form_2_PARSE(e,'NO_RELEASE')
    values = strtok(a[2],'|', /EXTRACT, ESC='\')
    if n_elements(values) ge 2 then begin
        type = 0
        if cw_form_2_PARSE(e, 'SET_VALUE', temp) then begin
...
process_integer:
    uextra = { VALUE: value }
    cw_form_2_LABEL, parent, nparent, e, frame
    if cw_form_2_PARSE(e, 'WIDTH', temp) then width = fix(temp) else width=6
...
ENDCASE
	...
	if cw_form_2_PARSE(e, 'TAG', value) then value = STRUPCASE(value) $
    ...
    if dflags and 1 then cw_form_2_BUILD, new, desc, cur, ids, lasttag ;Begin new
...
Function cw_form_2_EVENT, ev		;Event handler for CW_FOR
  COMPILE_OPT hidden
widget_control,   ev.id, GET_UVALUE=u, /NO_COPY  ;What kind of widget?
if u.type eq 1 then begin	;Droplist?  (can't get value)
    v = ev.index
    u.value = v
endif else if u.type eq 6 then begin
    v = WIDGET_INFO(ev.id, /LIST_SELECT)
    IF N_ELEMENTS(v) GT 1 THEN BEGIN
      newu = CREATE_STRUCT('type', u.type, 'base', u.base, 'tag',u.tag, 'next', u.next, 'quit',u.quit, 'value',v)
      u=newu
    ENDIF ELSE u.value = v
...
back_in: ret= { id: u.base, top: ev.top, handler: 0L, $
		tag: u.tag, value: u.value, quit: u.quit} ;Our value (v endret til u.value)
toss: widget_control, ev.id, SET_UVALUE=u, /NO_COPY	;Save new value...
return, ret
end			;cw_form_2_EVENT


Pro cw_form_2_SETV, id, value	;In this case, value = { Tagname : value, ... }
  COMPILE_OPT hidden
...
Function cw_form_2_GETV, id	;Return value of a cw_form_2 widget.
  COMPILE_OPT hidden
...
FUNCTION cw_form_2, parent, desc, $
	COLUMN = column, $
...
;  ON_ERROR, 2						;return to caller
  ...
  IF (NOT KEYWORD_SET(uname)) THEN UNAME='cw_form_2_UNAME'
  ...
  if n_params() eq 1 then begin
	...
	p = WIDGET_BASE(TITLE=title, XOFFSET=scrsize(0)*0.3, YOFFSET=scrsize(1)*0.3, Column = column, row=row, $
      ...
      if n_elements(ysize) gt 0 then WIDGET_CONTROL, p, YSIZE=ysize
  ...
  if n_elements(focusno) le 0 THEN focusno=0
  cw_form_2_BUILD, base, desc, 0, ids, lasttag
  widget_control, base, EVENT_FUNC='cw_form_2_EVENT', $
	FUNC_GET_VALUE='cw_form_2_GETV', PRO_SET_VALUE='cw_form_2_SETV'

  if n_params() eq 1 then begin		;Modal?
	WIDGET_CONTROL, p, /realize
	IF n_elements(focusno) GT 0 THEN BEGIN
	 WIDGET_CONTROL, ids(focusno), /INPUT_FOCUS
	ENDIF
	XMANAGER, 'cw_form_2', p, EVENT_HANDLER='cw_form_2_MODAL_EVENT'
        v = TEMPORARY(*handle)
	PTR_FREE, handle
